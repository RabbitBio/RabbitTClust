...
inline void printUsage(){
	...
	fprintf(stdout, "	-F <string>\t: sketch function, includes MinHash, WMH, OMH, HLL\n"); 
	...
	fprintf(stdout, "\t ./clust -l -d 0.05 -t 48 -F MinHash -o result.out -i viralList\n");
	fprintf(stdout, "\tfor generator cluster from exist MST:\n");
	fprintf(stdout, "\t ./clust -f -d 0.05 -i viralListMinHashGenomeInfo viralListMinHashMSTInfo -o result.out\n");
