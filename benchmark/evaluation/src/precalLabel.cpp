...
{	
	...
	while(getline(fs0, line))
	{
		...
		ss >> assembly_accession >> species_taxid >> genomeName;
		//cerr << species_taxid << endl;
	...
	cerr << "the groundTruthClustNumber size(not for this dataset) is: " << groundTruthClustNumber.size() << endl;
